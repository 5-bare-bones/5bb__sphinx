...
`sphinx entry add phrase {name} [--length {}] [--separator {}] [--include {}] [--exclude {}] [--list {}]`
...
TODO: test date / valid and invalid
...
```
sphinx entry add phrase Sample --length 5 --separator / --include atoll, sphinx
```

Passphrase using a word list (default):
```
sphinx entry add phrase Sample --length 7 --list WordList
```

Passphrase using a syllable list:
```
sphinx entry add phrase Sample --length 12 --separator = --list SyllableList
```
