...
`sphinx vault import {manager-name} [-e|--erase] [-p|--path]`
...
Import:
```
sphinx vault import {manager-name} --path {path_to_file}
```

Import and erase the file:
```
sphinx vault import {manager-name} --erase --path {path_to_file}
```
